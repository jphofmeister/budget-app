...
export const noFunctionAvailable = () => {
  console.log(
    componentName,
    getDateTime(),
    "A function wasn't passed as a props when it needed to be."
  );
};
...
export const hasNonEmptyProperty = (objectItem, propertyName) => {
  ...
  if (typeof objectItem === "object") {
    if (
      objectItem.hasOwnProperty(propertyName) === true &&
      isEmpty(objectItem[propertyName]) === false
    ) {
...
export const validateMilitaryTime = timeEntered => {
  ...
  if (typeof formatToString(timeEntered) === "string") {
    ...
    if (timeEnteredString.length !== 4) {
      ...
      if (
        isNaN(parseInt(timeEnteredString.charAt(0))) === true ||
        isNaN(parseInt(timeEnteredString.charAt(1))) === true ||
        isNaN(parseInt(timeEnteredString.charAt(2))) === true ||
        isNaN(parseInt(timeEnteredString.charAt(3))) === true
      ) {
        validTimeFormat = false;
      }
...
export const hasDecimalPlaces = (value, decimalPlaces) => {
  ...
  if (isEmpty(value) === true || isNaN(formatTrim(value)) === true) {
    ...
    if (
      isEmpty(valueToTest) === true ||
      isNaN(valueToTest) === true ||
      (isEmpty(currentDecimalPlaces) === false && valueDecimals.length > currentDecimalPlaces)
    ) {
      return false;
    } else {
      return true;
  ...
  if (Array.isArray(sortedArray) === true) {
    if (typeof sortedArray[0][sortProperty] === "number") {
      ...
      sortedArray.sort((a, b) => {
        const aProperty = removeArticlesFromBeginning(a[sortProperty]);
        const bProperty = removeArticlesFromBeginning(b[sortProperty]);
  ...
  if (allowLogging === true && databaseAvailable !== false) {
    ...
    fetch(url, {
      ...
      .then(results => {
        ...
        if (response.ok !== true) {
          addErrorLog(baseURL, databaseAvailable, {
            operation: `${operationValue} SQL Server`,
...
export const addErrorLog = (
  ...
  if (allowLogging === true && databaseAvailable !== false) {
    const operationValue = "Add Error Log";
