...
export const calculateDate = (date, dateType, amount) => {
  ...
  const amountToAdd = isEmpty(amount) === false ? parseFloat(amount) : 0;
...
export const displayMonthName = dateToDisplay => {
  ...
  const newDisplayDate = new Date(dateToDisplay);

  const month = newDisplayDate.toLocaleString("default", { month: "long" });
...
export const getStartOfMonth = currentDate => {
  const newStartOfMonth = new Date(currentDate);
...
export const getEndOfMonth = currentDate => {
  const newEndOfMonth = new Date(currentDate);

  const month = newEndOfMonth.getMonth();
...
export const checkIsSameDay = (date1, date2) => {
  ...
  const startOfDate2 = new Date(date2).setHours(0, 0, 0, 0);
...
export const displayDay = (dateToDisplay, removeLeadingZeroes) => {
  ...
  if (isEmpty(formattedDate) === false) {
    ...
    // * Day
    const dd = formatToString(formattedDate).substring(8, 10);
