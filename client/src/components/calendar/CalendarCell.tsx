...
const CalendarCell = props => {
  ...
  const componentName = "CalendarCell";

  // * isEmpty doesn't work on date type because typeof props.day is object and Object.keys(props.day).length is 0. -- 07/11/2024 JH
  const day = props.day !== null && props.day !== undefined && props.day !== "" ? props.day : null;
  const currentRangeStart =
    isEmpty(props.currentRangeStart) === false ? props.currentRangeStart : null;

  const currentMonth = getMonth(currentRangeStart);

  const calendarCellClasses = classnames("calendar__td", {
