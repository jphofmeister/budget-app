...
const Calendar = props => {
  ...
  const currentRangeStart =
    isEmpty(props.currentRangeStart) === false ? props.currentRangeStart : null;
  const calendarEvents = isEmpty(props.calendarEvents) === false ? props.calendarEvents : null;
  ...
  useEffect(() => {
    if (isEmpty(currentRangeStart) === false) {
      const allDaysInMonthView = getAllDaysOfMonth(currentRangeStart);

      const newWeeksInMonth = divideDaysIntoWeeks(allDaysInMonthView);
  ...
  const getAllDaysOfMonth = dayOfThisMonth => {
    const currentDay = calculateDate(dayOfThisMonth);
    ...
    const endDayIndex = endOfMonth.getDay();

    // * If the first of the month is not on Sunday, get the last days of the previous month for this week. -- 07/10/2024 JH
    if (startDayIndex > 0) {
      const daysToSubtract = startDayIndex * -1;

      const convertedStartDate = startOfMonth.toLocaleDateString("en-US");
    ...
    if (endDayIndex < 6) {
      ...
      const convertedEndDate = endOfMonth.toLocaleDateString("en-US");
  ...
  const checkIsInDateRange = (dateToCompare, dateRangeStart, dateRangeEnd) => {
    const thisDayIndex = dateToCompare.getDay();
    ...
    if (checkIsSameDay(dateToCompare, dateRangeStart)) {
      ...
      if (thisDayIndex === 0) {
        const dateRange = getEachDayOfInterval(dateRangeStart, dateRangeEnd);
    ...
    <div className="calendar">
      <div className="calendar__thead">
        <div className="calendar__tr">
          {weekdays.map(day => {
            return (
              <div className="calendar__th" key={day}>
                {day}
              </div>
            );
          })}
      ...
      {isNonEmptyArray(weeksInMonth) === true && isNonEmptyArray(calendarEvents) === true ? (
        <div className="calendar__tbody">
          ...
          {weeksInMonth.map((week, index) => {
            ...
            const thisWeeksEvents = week.flatMap(day => {
              const thisDaysEvents = calendarEvents.filter(
                event =>
                  checkIsInDateRange(day, event.calendarStartDate, event.calendarEndDate) === true
              );
